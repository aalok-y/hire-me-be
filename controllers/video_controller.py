...
UPLOAD_DIR = Path("/home/quantum/Downloads/temp/merged-interview-video")
...
async def save_merged_video(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error saving merged video: {str(e)}"
        )
...
async def get_interview_video(application_id: str, user_id: str):
    ...
    return FileResponse(
        path=str(file_path), media_type="video/webm", filename=file_path.name
    )
...
async def get_interview_video(application_id: str, user_id: str, request: Request):
    """
    ...
    try:
        ...
        if not application_id or not user_id:
            raise HTTPException(
                status_code=400, detail="application_id and user_id are required"
            )
        ...
        if not user_dir.exists():
            ...
            raise HTTPException(
                status_code=404, detail=f"No interviews found for user {user_id}"
            )
        ...
        if not matching_files:
            logger.warning(
                f"No video files found for app_id={application_id} in {user_dir}"
            )
            raise HTTPException(
                status_code=404,
        ...
        logger.info(
            f"Found {len(matching_files)} video file(s), returning latest: {file_path.name}"
        )
        ...
        if not os.access(file_path, os.R_OK):
            ...
            raise HTTPException(
                status_code=403, detail="Permission denied accessing video file"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving interview video: {str(e)}"
        )


def handle_range_request(
    file_path: Path, range_header: str, file_size: int
) -> StreamingResponse:
    """
    ...
    try:
        ...
        logger.info(
            f"Serving range: bytes {start}-{end}/{file_size} ({content_length} bytes)"
        )
...
async def list_interview_videos(user_id: str):
    ...
    try:
        ...
        if not user_dir.exists():
            return {"user_id": user_id, "videos": [], "total_count": 0}

        # Get all video files
        video_files = sorted(
            user_dir.glob("interview_*.webm"),
...
async def delete_interview_video(application_id: str, user_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error deleting interview video: {str(e)}"
        )
